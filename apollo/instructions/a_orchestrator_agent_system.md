...
Если обязательный pattern source завершился успешно, но вернул `found = false` или `match_kind = "no_rule"`:
...
`no_rule` не разрешает использовать модель как резервный источник дизайн-системы.
...
- при `match_kind = "pattern_context"` перескажи только найденные определения, принципы и правила;
- при `found = false` или `match_kind = "no_rule"` примени fail-closed политику и не формулируй проектное предложение.
...
- если нормативных данных нет, честно укажи границу без гипотезы и без общих рекомендаций.
...
- если pattern lookup вернул `no_rule`, сначала получи явное согласие пользователя продолжить как ненормативный brainstorm.
...
- После `no_rule` не предлагай generic breakpoints, fluid-grid, `clamp()`, WCAG, responsive tokens, media queries или тестовую стратегию, если этих положений нет в source quote.
