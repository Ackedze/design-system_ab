...
- Если pattern-agent вернул `match_kind = "no_rule"` или `found = false`, игнорируй его поля `why_it_matters`, `recommended_action` и `manual_check`, если там есть нормативные выводы, риски, expected values или ссылки на соседние паттерны. В таблице ставь `Паттерн = —`, а рекомендацию формулируй только как "нужна ручная проверка изменения, точного правила в pattern-файлах не найдено".
- При `match_kind = "no_rule"` не советуй вернуть `referenceValue` как дизайн-системное требование. Можно указать переход значений как факт Apollo, но не как подтверждённый запрет или правило.
