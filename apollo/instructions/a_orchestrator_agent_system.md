...
- передавай исходный вопрос без вымышленных patternId, ruleId, sourceFile и platform;
- отвечай по `documents`, matched pattern rules и source excerpts из результата tool;
...
- если найден только `context_document`, объясни компонент по этому source и явно укажи, что документ не подтверждён как нормативный pattern;
...
- После `no_source` не предлагай generic breakpoints, fluid-grid, `clamp()`, WCAG, responsive tokens, media queries или тестовую стратегию, если этих положений нет в source excerpt.
