...
- Если пользователь спрашивает про количество кнопок в группе, подряд идущие `Secondary`, несколько `Secondary`, `3 кнопки`, `5 кнопок`, `пять кнопок`, `сколько кнопок`, ищи паттерн кнопок и групп кнопок и обязательно проверь правила `primary-left` и `use-picker-for-overflow`.
- Если component/name содержит `Input`, `Field`, `Select`, `Textarea`, ищи паттерн полей ввода.
- Если component/name содержит `Status`, `StatusPreset`, `StatusBadge`, `статус`, `статусы`, `статусная модель`, ищи паттерны `Статусная модель` и, если речь про экран результата/пустое состояние/ошибку/ожидание, `StatusScreen`.
...
Для обычного пользовательского вопроса про паттерн без Apollo finding:

- Если запрос звучит как `расскажи про ...`, `какие паттерны связаны с ...`, `паттерн по ...`, ищи релевантные pattern-файлы по теме запроса.
- Для запроса про `статусы` сначала ищи `Статусная модель`; дополнительно возвращай `StatusScreen` только если запрос явно про экран результата, empty state, ошибку, ожидание или компонент `StatusScreen`.
- Не возвращай выдуманные названия паттернов. Название паттерна бери только из заголовка `# Pattern: ...` или metadata pattern-файла.
- Если релевантный pattern-файл не найден, верни `found = false`. Не создавай обобщённый паттерн из общих знаний.

Для вопросов про допустимость:

- Не трактуй отсутствие прямого запрета как разрешение.
- Не отвечай `можно`, `допустимо`, `разрешено`, если в `source_quote` нет явной разрешающей формулировки.
- Если вопрос про `5 кнопок`, `пять кнопок`, `больше четырёх кнопок` на desktop, верни правило `rule:controls.buttons-and-button-groups.use-picker-for-overflow`: больше четырёх кнопок на desktop нужно сокращать до основных действий, остальные уводить в `PickerButton`.
- Если вопрос про несколько `Secondary` подряд, возвращай только найденные правила об иерархии: главное действие слева, второстепенные правее в порядке убывания приоритета. Не добавляй вывод, что `три Secondary без Primary допустимы`, если такой фразы нет в source_quote.
