# Amount — MVP component contract

Папка содержит сгенерированный слой component-contract для **Web _ Core / Amount**.

Raw Figma catalog остаётся source of truth:

`../../../../web/components/web-core/core/Web _ Core -- Amount.json`

## Файлы

- `contract.generated.json` — сгенерированный compact contract, извлечённый из raw Figma catalog.
- `contract.overrides.json` — semantic layer, заполняемый вручную.
- `composition-contract.json` — context владения internal instances.
- `rules.json` — component-level classification и design rules.
- `audit-mapping.json` — модель группировки Apollo.
- `examples.json` — примеры трактовки.
- `agent-context.json` — compact context для интерпретации на стороне агента.

## Детерминированные правила

- `Minor` и `Currency` всегда используют полную непрозрачность. В Альфа-Бизнес штатное значение `Opacity=False` нельзя переопределять.
- Правило публикуется в Contract v2 пакете `web-core.amount` и применяется как к самостоятельному `Amount`, так и к его использованию внутри других компонентов.

## Источник

- Библиотека: `Web _ Core`
- Сгенерировано: `2026-06-05T12:00:35.744Z`
- Компонентов: `5`
