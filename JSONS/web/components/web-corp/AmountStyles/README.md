...
`🔒 [D] AmountHeadline` используется только на desktop, `🔒 [M] AmountHeadline` — только на mobile-web; `🔒 AmountParagraph` применяется на обеих платформах. Amount всегда остаётся в одну строку. Контейнер должен обеспечить достаточную ширину; при её недостатке Amount клипуется контейнером.
