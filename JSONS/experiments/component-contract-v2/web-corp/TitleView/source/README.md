...
- В `View=xLarge` `TitleStatus` может использоваться только при включённом boolean-свойстве `Status` у того же `TitleView`. Для `Large`, `Medium` и `Small` слот `TitleStatus` недоступен; сохранённое скрытое значение свойства не является использованием слота и не проверяется.
