...
- универсальный web-breakpoint не уточняй для каждого компонента: `MobileWeb` — `320–767 px`, `Desktop` — от `768 px` по `rule:layout.adaptive-alfa-business.breakpoints`; спрашивай только про исключения и специальное responsive-поведение;
