...
function buildSelectors(generatedContract) {
  ...
  return {
    ...
    'text.title-and-subtitle': {
      ...
      from: 'host.title-view',
      where: {
        semanticRoleOrLayerName: { op: 'oneOf', values: ['Title', 'Subtitle'] },
      },
      occurrence: 'all',
      orderBy: 'document',
    },
    'paint.title-subtitle-and-paintme': {
      scope: 'descendants',
      from: 'host.title-view',
      where: {
        semanticRoleOrLayerName: { op: 'oneOf', values: ['Title', 'Subtitle', 'PaintMe'] },
      },
      occurrence: 'all',
      orderBy: 'document',
    },
...
function buildAuthoredRules(sourceRuleById) {
  const definitions = [
    ...
    authored(
      ...
      'enforced',
      { host: 'host.title-view', targets: 'tree.title-view' },
      { op: 'matchesEffectiveBaseline', properties: ['layout.*', 'stroke', 'radius', 'opacity', 'effects.*', 'blendMode'] },
    ...
    authored(
      ...
      'title-and-subtitle-typography-is-fixed',
      'error',
      'enforced',
      { host: 'host.title-view', targets: 'text.title-and-subtitle' },
      { op: 'matchesEffectiveBaseline', properties: ['styles.text'] },
      ['diff.property', 'diff.actual', 'baseline.effective', 'ownership.owner'],
      ['matchesEffectiveBaseline'],
      ['restore-effective-baseline'],
    ),
    authored(
      sourceRuleById,
      'component:web-corp.title-view.title-and-subtitle-color-is-fixed',
      'title-and-subtitle-color-is-fixed',
      'error',
      'enforced',
      { host: 'host.title-view', targets: 'paint.title-subtitle-and-paintme' },
      { op: 'matchesEffectiveBaseline', properties: ['fill', 'styles.fill'] },
      ['diff.property', 'diff.actual', 'baseline.effective', 'ownership.owner'],
      ['matchesEffectiveBaseline'],
      ['restore-effective-baseline'],
    ),
