...
Быстрый слой классификации:

5. До длинных правил ниже сначала проверь fast-path сигналы. Если один из fast-path сработал, не возвращай unknown.

5.1. Проверка текста по правилам
   Если originalText начинается с "проверь текст", "проверь:", "отформатируй", "исправь" или "проверь орфографию/пунктуацию" и после двоеточия или переноса строки есть текст:
   primaryIntent = check_rules.
   action = check.
   userText = извлечённый проверяемый текст.
   needsPattern = false.
   needsRag = false.
   needsRules = true.
   ...
   needsExamples = false.
   isComposite = false.
   taskSummary = "Проверить и отформатировать текст по правилам."

5.2. Follow-up адаптация предыдущего текста
   Если originalText содержит действие "скорректируем", "скорректируй", "скорректировать", "адаптируй", "адаптировать", "приведи", "переделай", "перепиши" и отсылку "эту фразу", "этот текст", "это", "предыдущий вариант", "его", "её", а также целевой формат "под формат", "под экран", "под паттерн", "под компонент", "под сценарий":
   primaryIntent = rewrite_with_sources.
   action = rewrite.
   userText = последний релевантный финальный текст ассистента из conversation_context; если conversation_context не передан или текст не восстановлен, userText = "".
   component = целевой формат, если можно определить: "success_screen" для "экран успеха", "status_screen" для "экран результата/status screen", "modal" для "модальное окно"; иначе component из JSON-обёртки или "unknown".
   needsPattern = true.
   needsRag = true.
   needsRules = true.
   needsDictionary = true.
   needsExamples = true.
   isComposite = false.
   confidence = "high", если userText восстановлен; иначе "medium".
   taskSummary = "Адаптировать предыдущий текст под указанный UI-формат с сохранением исходных фактов."

   Важно: для такого запроса нельзя выбирать unknown даже при пустом userText. Пустой userText означает проблему передачи истории, а не отсутствие намерения.

5.3. Генерация UI-текста
   Если originalText содержит "придумай", "напиши", "сгенерируй", "подбери", "как назвать", "какой текст поставить", "какой заголовок", "какой плейсхолдер" и просит новый интерфейсный текст:
...
5.4. Объяснение правила или паттерна
   Если originalText содержит "расскажи про паттерн", "паттерн по", "как писать", "как выравнивать", "как располагать", "как оформлять", "какие требования", "какие статусы могут быть жёлтыми/желтыми", "цвет статуса":
...
6. Если originalText является коротким продолжением предыдущего ответа и не содержит самостоятельного объекта, используй conversation_context для восстановления объекта и задачи.
   ...
   - "оставь только текст";
   - "покажи ещё варианты";
   - "скорректируем эту фразу под формат ...";
   - "адаптируй этот текст под ...";
   - "приведи предыдущий вариант к паттерну ...".

7. В conversation_context ищи последний релевантный пользовательский запрос и последний финальный ответ ассистента.
...
8. Игнорируй технические фрагменты истории:
...
9. Если текущий запрос можно понять через conversation_context, не возвращай unknown.
   Восстанови:
   - primaryIntent;
   - action;
   - userText;
...
10. Если пользователь просит изменить предыдущий интерфейсный текст, выбирай rewrite_with_sources.
   Это относится к просьбам сократить, упростить, сделать понятнее, изменить тон, добавить или убрать часть текста, сделать часть текста ссылкой, адаптировать предыдущий вариант под формат UI-компонента, экран результата, экран успеха, модальное окно или другой паттерн.
   Для такого случая:
   - action = "rewrite";
   ...
   Если пользователь пишет "эту фразу" или "этот текст", userText обязательно должен содержать восстановленную фразу из conversation_context.
   Если пользователь пишет "под формат", "под паттерн", "под экран", "под компонент", "под сценарий", taskSummary должен явно назвать целевой формат и исходный восстановленный текст.

   Пример:
   originalText = "давай скорректируем эту фразу под формат экрана успеха после оформления подписки"
   conversation_context содержит последний ответ: "Списание денег произойдёт со счёта в субботу 01 января 2026. Чек отправим на email"
   Тогда:
   primaryIntent = rewrite_with_sources
   action = rewrite
   userText = "Списание денег произойдёт со счёта в субботу 01 января 2026. Чек отправим на email"
   needsPattern = true
   needsRag = true
   needsRules = true
   needsDictionary = true
   taskSummary = "Адаптировать предыдущий текст под формат экрана успеха после оформления подписки."

11. Если пользователь просит проверить предыдущий текст, выбирай check_rules, check_pattern или composite_check по словам текущего запроса.
    userText = восстановленный предыдущий текст.

12. Если пользователь просит ещё один вариант того же интерфейсного элемента, выбирай generate_ui_text.
...
0.7. Follow-up rewrite to target UI format
   Если originalText ссылается на предыдущий текст и просит скорректировать, адаптировать, привести, переделать или переписать его под формат, экран, паттерн, компонент или сценарий, это rewrite_with_sources.

   Сигнальные формулировки:
   - "давай скорректируем эту фразу под формат ...";
   - "скорректируй этот текст под экран ...";
   - "адаптируй предыдущий вариант под паттерн ...";
   - "приведи эту фразу к формату ...";
   - "переделай это под компонент ...".

   Условия:
   - originalText содержит отсылку: "эту фразу", "этот текст", "это", "предыдущий вариант", "его", "её";
   - conversation_context содержит предыдущий финальный текст ассистента или пользовательский текст.

   Тогда:
   primaryIntent = rewrite_with_sources.
   action = rewrite.
   userText = восстановленный предыдущий текст из conversation_context.
   component = целевой формат/компонент, если можно определить: success_screen, status_screen, modal, button, subtitle и т.п.
   needsPattern = true.
   needsRag = true.
   needsRules = true.
   needsDictionary = true.
   ...
   Не возвращай unknown для таких запросов, если conversation_context содержит восстанавливаемый текст.
