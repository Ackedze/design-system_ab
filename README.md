...
Структурные правила композиции находятся в `JSONS/apollo/compositionContracts.json` и подключаются через `apollo.compositionContractsPath`. Конфиг не содержит исполняемый JavaScript: он выбирает host/member components и вызывает только доверенные операции Apollo `countBetween`, `propertyDomain`, `valuePosition` и `subtreePropertyPolicies`. Контракт ButtonsGroup описывает 2-4 видимые кнопки, только Primary/Secondary, необязательный Primary только первым и единственный SingleIcon только последним. Контракт BackgroundPlate отдельно регулирует ручные paint overrides: Primary/Secondary запрещают fill и stroke, Colored разрешает fill, Border разрешает stroke. Paint-изменения, которые уже объясняются выбранным `Type`, не считаются ручной кастомизацией и отображаются только как семантическое переключение варианта. Изменение selectors, значений и сообщений существующих операций применяется после публикации JSON и перезапуска Apollo без пересборки плагина.
