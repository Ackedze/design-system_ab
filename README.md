...
Пакеты `AmountStyles` и Core `Amount` отдельно проверяют два способа изменения прозрачности `Minor/Currency`: прямой layer opacity должен оставаться `1`, а component property `Opacity` — `False`. Если property-переключение производно меняет layer opacity, Apollo показывает изменение property без дублирующего layer finding. `AmountStyles` также использует `sharedValueConstraint.strategy=all-visible-targets-equal`: все видимые текстовые части суммы, кроме `Addon`, должны иметь один фактический цвет. Единая контекстная перекраска разрешена, частичная считается нарушением. Форматные и сценарные правила с `checkType=llm` остаются контекстом агента и не повышаются до автоматических ошибок без наблюдаемого evidence.
