...
Пакеты `AmountStyles` и Core `Amount` отдельно проверяют два способа изменения прозрачности `Minor/Currency`: прямой layer opacity должен оставаться `1`, а component property `Opacity` — `False`. Если property-переключение производно меняет layer opacity, Apollo показывает изменение property без дублирующего layer finding; контрактная remediation сбрасывает именно property в `Opacity=False`. `AmountStyles` также использует `sharedValueConstraint.strategy=all-visible-targets-equal`: все видимые текстовые части суммы, кроме `Addon`, должны иметь один фактический цвет. Единая контекстная перекраска разрешена, частичная считается нарушением. Форматные и сценарные правила с `checkType=llm` остаются контекстом агента и не повышаются до автоматических ошибок без наблюдаемого evidence.

Экспериментальный Contract v2 собирает 27 package-профилей, включая supplemental-пакеты `web-core/TagGroup` и `web-corp/PaymentMaskedNumber`. Компилятор сохраняет target scope исходного правила: `target.component(s)` ограничивает host, `target.layer(s)` адресует semantic layer, `target.slots` — слоты, `target.placeholders` — конкретные placeholders, а `layer=root` выбирает только корень подходящего компонента. Сборка завершается ошибкой, если любой из этих target теряется при компиляции. TagGroup проверяет соответствие размера вложенного Tag размеру host-компонента, PaymentMaskedNumber проверяет fill Major/Minor относительно effective baseline. TitleView использует отдельные правила для типографики и цвета Title/Subtitle, а BackgroundPlate Border разрешает дизайнерское управление stroke при обязательном отсутствии видимого fill. Массив `uniformProperties` компилируется в независимый `allEqual` RuleIR для каждого свойства: поэтому Benefits отдельно проверяет `Background`, `CardAxis`, `Compact`, `GraphicPosition` и `Title` у вложенных BenefitCard и указывает конкретный outlier.

Правило может явно запросить direct baseline выбранного host-варианта через `classification.baselineSource="host-variant"`; компилятор переносит это значение в `assert.baselineSource`. Сейчас этот режим используется для fill `StatusPreset`, чтобы вложенный `Label` сверялся с Type/Style родительского preset, а не с самостоятельно выбранным Size вложенного Status. Остальные baseline-правила продолжают использовать materialized effective baseline.
...
Package-level `composition-contract.json#/manual/contractOwnership` может объявить `nestedPackages` с режимом `host-contract`. Компилятор сохраняет эту политику в `facts.contractOwnership`: host package полностью оценивает соответствующий nested subtree, а Apollo не запускает вложенный package повторно. `AmountStyles` использует эту схему для `web-core.amount`, чтобы typography/color/opacity/layout сравнивались только с effective baseline выбранного preset Style. `Table Wide [D]` тем же способом владеет вложенными `web-core.amount` и `web-corp.payment-masked-number` в materialized состояниях `Text Presets=Amount/Account`: штатное оформление выбранного preset не считается кастомизацией, а ручное изменение листового слоя проверяется table effective baseline. Standalone-пакеты остаются самостоятельными contract scopes вне этих host-компонентов.
